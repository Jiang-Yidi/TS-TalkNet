...
class ts_talkNetModel(nn.Module):
    def __init__(self):
        ...
        self.visualConv1D    = visualConv1D(out = 192)   # Visual Temporal Network Conv1d

        # Audio Temporal Encoder 
        self.audioEncoder  = audioEncoder(layers = [3, 4, 6, 3],  num_filters = [16, 32, 64, 192])
        ...
        self.crossA2V = attentionLayer(d_model = 192, nhead = 8)
        self.crossV2A = attentionLayer(d_model = 192, nhead = 8)
        ...
        self.selfAV = attentionLayer(d_model = 192 * 3, nhead = 8)
    ...
    def forward_audio_visual_backend(self, x1, x2, x3): 
        ...
        x_a_c, x_s_c = self.forward_speaker_av_cross_attention(x1,x3)
        x_avs = torch.cat((x_a_c, x2, x_s_c), 2)
        x_avs = self.selfAV(src = x_avs, tar = x_avs)       
        x_avs = torch.reshape(x_avs, (-1, 192 * 3))
        return x_avs   


    def forward_audio_backend(self,x):
        x = torch.reshape(x, (-1, 192))
        return x
